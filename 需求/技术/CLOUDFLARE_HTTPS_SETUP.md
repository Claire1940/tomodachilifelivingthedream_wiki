...
| URL                               | 状态            | 展示次数 |
| --------------------------------- | --------------- | -------- |
| `http://www.wwe2k26.wiki/`    | ❌ HTTP 版本    | 38 次    |
| `http://wwe2k26.wiki/`        | ❌ HTTP 无 www  | 18 次    |
| `https://www.wwe2k26.wiki/`   | ✅ HTTPS + www  | 11 次    |
| `https://www.wwe2k26.wiki/fr` | ✅ HTTPS 多语言 | -        |

### 根本原因

1. **Dokploy 域名配置问题**：所有域名都配置为 HTTP (Port: 3000)，Cert: none
2. **缺少 HTTP → HTTPS 强制重定向**：用户可以通过 HTTP 访问网站
3. **缺少非 www → www 重定向**：`wwe2k26.wiki` 和 `www.wwe2k26.wiki` 都可以访问
4. **SEO 影响**：
   - 重复内容问题
   - 分散页面权重
   - 降低 SEO 排名
...
- **期望的主域名**: `https://www.wwe2k26.wiki`（HTTPS + www）

### 当前 Dokploy 配置

3 个域名配置：

1. `project1-wwe2k26wiki-vbzibg-35e314-43-130-44-149.traefik.me` - HTTP, Port: 3000, Cert: none
2. `wwe2k26.wiki` - HTTP, Port: 3000, Cert: none
3. `www.wwe2k26.wiki` - HTTP, Port: 3000, Cert: none

### 当前环境变量

```bash
NEXT_PUBLIC_SITE_URL=https://www.wwe2k26.wiki ✅ 正确
```
...
- 这会处理：
  - `http://www.wwe2k26.wiki` → `https://www.wwe2k26.wiki`
  - `http://wwe2k26.wiki` → `https://wwe2k26.wiki`
...
以 `animetacticalsimulator.wiki` 为例：
...
Request URL: https://animetacticalsimulator.wiki/*

Then:
  Target URL: https://www.animetacticalsimulator.wiki/${1}
  Status code: 301
  ☑ Preserve query string
...
- 这会将 `https://yourdomain.wiki/*` 重定向到 `https://www.yourdomain.wiki/*`
- 使用 301 永久重定向，告诉搜索引擎这是永久性的改变
- 保留查询参数和路径
- 不会匹配 `www.yourdomain.wiki`，避免重定向循环
...
```
错误: animetacticalsimulator.wiki/*
正确: https://animetacticalsimulator.wiki/*
```
...
| 域名                                                              | 操作      | 原因                           |
| ----------------------------------------------------------------- | --------- | ------------------------------ |
| `www.wwe2k26.wiki`                                            | ✅ 保留   | 主域名                         |
| `wwe2k26.wiki`                                                | ✅ 保留   | 用于重定向到 www               |
| `project1-wwe2k26wiki-vbzibg-35e314-43-130-44-149.traefik.me` | ⚠️ 可删除 | Dokploy 默认域名（如果不需要） |
...
```bash
NEXT_PUBLIC_SITE_URL=https://www.wwe2k26.wiki ✅
```
...
curl -I http://www.wwe2k26.wiki
# 期望: 301/308 重定向到 https://www.wwe2k26.wiki

# 测试 HTTP 非 www → HTTPS www
curl -I http://wwe2k26.wiki
# 期望: 301 重定向到 https://www.wwe2k26.wiki

# 测试 HTTPS 非 www → HTTPS www
curl -I https://wwe2k26.wiki
# 期望: 301 重定向到 https://www.wwe2k26.wiki

# 测试最终 URL
curl -I https://www.wwe2k26.wiki
# 期望: 200 OK
```

### 2. 使用浏览器测试

在浏览器中访问以下 URL，确认都重定向到 `https://www.wwe2k26.wiki`：

- ✅ `http://www.wwe2k26.wiki`
- ✅ `http://wwe2k26.wiki`
- ✅ `https://wwe2k26.wiki`
...
```
http://wwe2k26.wiki
  → 301 → https://wwe2k26.wiki (Cloudflare HTTPS 重定向)
  → 301 → https://www.wwe2k26.wiki (Cloudflare www 重定向)
  → 200 OK
```
...
```bash
# 检查 SSL 证书
openssl s_client -connect www.wwe2k26.wiki:443 -servername www.wwe2k26.wiki
```
...
- [ ] `http://www.wwe2k26.wiki` → `https://www.wwe2k26.wiki` ✅
- [ ] `http://wwe2k26.wiki` → `https://www.wwe2k26.wiki` ✅
- [ ] `https://wwe2k26.wiki` → `https://www.wwe2k26.wiki` ✅
- [ ] `https://www.wwe2k26.wiki` 返回 200 OK ✅
...
- ✅ 所有流量统一到 `https://www.wwe2k26.wiki`
- ✅ 消除重复内容问题
- ✅ 集中页面权重和排名
- ✅ 提升搜索引擎信任度
...
**症状**：`https://wwe2k26.wiki` 不重定向到 `https://www.wwe2k26.wiki`
...
**症状**：`http://www.wwe2k26.wiki` 不重定向到 HTTPS
